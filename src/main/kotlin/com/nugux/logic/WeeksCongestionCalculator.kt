...
object WeeksCongestionCalculator {
    fun getWeeksCongestion(baseCongestion: Double): List<Double> {
        val congestionOfWeek = listOf(
            baseCongestion,
            baseCongestion,
            baseCongestion,
            baseCongestion,
        ...
        val averageCongestionOfWeek = listOf(4.0, 1.0, 0.5, 1.0, 2.0, 3.0, 4.0)
    ...
    private fun customNormalization(data: List<Double>): List<Double> {
        ...
        return data.map { (((it - min) / (max - min)) / 1.25 + 0.2) * 5 }
    }
}